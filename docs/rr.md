...
| MapControl | 0.00 | 你靠站位/枪线为团队创造了多少空间 | 当前保持 shadow；上游 official fields 稳定后再按 0.15 → 0.20–0.25 ramp |
...
`mapControl: 0.00` 是当前 shadow 状态。职业样本验证 official fields 稳定后，先升到 `0.15`，
再考虑 `0.20–0.25`。不要从 proxy 字段直接进入正式评分。
...
- frozen pro baseline：`computeFrozenProBaselineRR` + `rrSixAccountProBaselineV0`（52 ZIP provisional v0）。

待实现：

- 扩到 100–200 张职业 map-demo 后冻结权威 v1；
- 用职业样本验证/校准 MapControl 和 UtilitySpatial 的 raw 量级；
- 将 DAK 的 strict ZIP schema 与本地导出器新增的 `positions-1s.lastPlaceName` / `replay.projectiles` 对齐。
